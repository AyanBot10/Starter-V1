...
module.exports = {
  ...
  start: async function({ event, args, api }) {
    ...
    let cook;
    try {
      if (!prompt || prompt.length <= 5) return api.sendMessage(event.chat.id, "Include a valid prompt");
      ...
      await api.sendPhoto(event.chat.id, polledImage, { caption: models[rando].name });
    } catch (err) {
      console.error(err);
      ...
      const options = {
        reply_markup: {
          inline_keyboard: [[{
            text: 'Try Again',
            callback_data: `/imagine ${prompt}`
          }]]
        },
        reply_to_message_id: event.message_id
      };

      api.sendMessage(event.chat.id, err.message, options);
    }
  },
  callback: async function({ event, api, ctx }) {
    const prompt = ctx.data.substring(9);
    let cook;
    try {
...
function formatPrompt(text) {
  ...
  if (regex.test(text)) {
    ...
    if (!versionRegex.test(text)) return { text, model: models[rando].model, rando };
...
async function generateImage({ text, model }) {
  try {
    const request = await axios.get(`https://api.prodia.com/generate?new=true&prompt=${encodeURIComponent(text)}&model=${encodeURIComponent(model)}&negative_prompt=${encodeURIComponent(`verybadimagenegative_v1.3, ng_deepnegative_v1_75t, (ugly face:0.8),cross-eyed,sketches, (worst quality:2), (low quality:2), (normal quality:2), lowres, normal quality, ((monochrome)), ((grayscale)), skin spots, acnes, skin blemishes, bad anatomy, DeepNegative, facing away, tilted head, {Multiple people}, lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worstquality, low quality, normal quality, jpegartifacts, signature, watermark, username, blurry, bad feet, cropped, poorly drawn hands, poorly drawn face, mutation, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, extra fingers, fewer digits, extra limbs, extra arms,extra legs, malformed limbs, fused fingers, too many fingers, long neck,mutated hands, polar lowres, bad body, bad proportions, gross proportions, text, error, missing fingers, missing arms, missing legs, extra digit, extra arms, extra leg, extra foot, zombie, elf ear, animal ears, cat ears, fox ears, dog ears, rabbit ears, atrist name, user name, multipul angle, split view, grid view, multipul shot`)}&steps=100&cfg=7&seed=${Date.now()}&sampler=${encodeURIComponent("DPM++ 2M Karras")}&aspect_ratio=${["square", "portrait", "landscape"][Math.floor(Math.random() * 3)]}`);
...
async function pollImage({ jobID }) {
  try {
    while (true) {
      const response = (await axios.get(`https://api.prodia.com/job/${jobID}`)).data;
